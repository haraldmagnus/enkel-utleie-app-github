...
export default function PaymentReminders() {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.PaymentReminder.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.PaymentReminder.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['paymentReminders'] });
      setShowDialog(false);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.PaymentReminder.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['paymentReminders'] });
    }
  });
  ...
  const handleEdit = (reminder) => {
    ...
    setFormData({
      rental_unit_id: reminder.rental_unit_id,
      tenant_email: reminder.tenant_email || '',
      amount: reminder.amount?.toString() || '',
      due_day: reminder.due_day?.toString() || '1',
      reminder_days_before: reminder.reminder_days_before?.toString() || '3',
      send_email: reminder.send_email !== false,
      send_push: reminder.send_push !== false,
    ...
    <div className="pb-24">
        ...
        {reminders.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center">
              ...
              >
                <Plus className="w-4 h-4 mr-1" /> Opprett påminnelse
              </Button>
            </CardContent>
          </Card>
        ) : (
                ...
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                      reminder.active ? 'bg-green-100' : 'bg-slate-100'
                    }`}>
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Forfaller den {reminder.due_day}. hver måned • 
                        Varsel {reminder.reminder_days_before} dager før
                      </p>
                      {reminder.tenant_email && (
                        <p className="text-xs text-slate-500">Til: {reminder.tenant_email}</p>
                      )}
                    </div>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingReminder ? 'Rediger påminnelse' : 'Ny betalingspåminnelse'}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(v) => setFormData({ ...formData, due_day: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              ...
              <Select 
                ...
                onValueChange={(v) => setFormData({ ...formData, reminder_days_before: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <DialogFooter>
              ...
              <Button 
                type="submit" 
                className="bg-blue-600 hover:bg-blue-700"
                disabled={createMutation.isPending || updateMutation.isPending || !formData.rental_unit_id || !formData.amount}
              >
                {editingReminder ? 'Oppdater' : 'Opprett'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
