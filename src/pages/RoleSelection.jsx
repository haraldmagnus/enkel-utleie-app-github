...
export default function RoleSelection() {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      try { localStorage.setItem('user_role_override', selectedRole); } catch (e) {}
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-100 to-white flex flex-col items-center justify-center p-6">
      ...
      <div className="w-full max-w-md space-y-4">
        ...
        <Button className="w-full mt-6 bg-blue-600 hover:bg-blue-700" disabled={!selectedRole || isLoading} onClick={handleContinue}>
          {isLoading ? t('loading') : t('continue')}
          <ArrowRight className="w-4 h-4 ml-2" />
        </Button>

        <div className="mt-2 flex justify-center">
          <button onClick={() => base44.auth.logout()} className="flex items-center gap-1 text-xs text-slate-400 hover:text-red-500 transition-colors">
        ...
        <p className="text-center text-xs text-slate-500 pt-2">
          {t('needHelpContactUs')}{' '}
          <a href="mailto:support@enkelutleie.com" className="text-blue-600 hover:underline">support@enkelutleie.com</a>
        </p>
      </div>
    </div>
  );
