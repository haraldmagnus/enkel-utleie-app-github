...
function LogRow({ log }) {
  ...
  const meta = (() => {
    try { return JSON.parse(log.meta_json || '{}'); } catch { return {}; }
  })();
    ...
    <div className={`border rounded-lg mb-2 ${log.level === 'ERROR' ? 'border-red-200' : log.level === 'WARN' ? 'border-amber-200' : 'border-slate-200'}`}>
      ...
      >
        ...
        <span className="text-xs text-slate-400 flex-shrink-0">
          {new Date(log.created_date).toLocaleString('no', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' })}
        </span>
        ...
        <div className="px-4 pb-4 border-t border-slate-100 pt-3 space-y-3">
          ...
          {Object.keys(meta).length > 0 && (
            <div>
              ...
              <pre className="bg-slate-50 rounded p-2 text-xs overflow-auto max-h-40 text-slate-700">
          ...
          {log.stack && (
            <div>
              ...
              <pre className="bg-red-50 rounded p-2 text-xs overflow-auto max-h-40 text-red-700">
                {log.stack}
              </pre>
            </div>
          )}
          ...
          {log.browser_info && (
            <p className="text-xs text-slate-400 truncate">Browser: {log.browser_info}</p>
          )}

          <Button size="sm" variant="outline" onClick={handleCopy} className="text-xs">
            <Copy className="w-3 h-3 mr-1" />
            {copied ? 'Kopiert!' : 'Kopier logg'}
          </Button>
        </div>
      )}
    </div>
...
export default function ErrorLogs() {
  ...
  const deleteOldMutation = useMutation({
    mutationFn: async () => {
      ...
      const old = logs.filter(l => l.created_date < cutoff);
      await Promise.all(old.map(l => base44.entities.ErrorLog.delete(l.id)));
    },
  ...
  if (user?.role !== 'admin') {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <p className="text-slate-500">Kun tilgjengelig for administratorer.</p>
      </div>
    );
  }
    ...
    <div className="pb-24 px-4 max-w-4xl mx-auto">
      <div className="py-4 border-b mb-4">
        ...
        <div className="flex gap-3 mt-2 text-sm">
          <span className="text-red-600 font-medium">{errorCount} feil</span>
          <span className="text-amber-600 font-medium">{warnCount} advarsler</span>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-40">
          ...
          <Input
            ...
            placeholder="Søk på Correlation ID..."
        ...
        >
          <Trash2 className="w-3 h-3 mr-1" /> Slett gamle (&gt;30d)
        </Button>
      </div>

      {isLoading ? (
        <p className="text-center text-slate-400 py-8">Laster...</p>
      ) : filtered.length === 0 ? (
        <Card>
