...
export default function AcceptCoLandlord() {
  ...
  const [state, setState] = useState('LOADING'); // LOADING, READY, ACCEPTING, DONE, ERROR, EXPIRED, ALREADY
  ...
  useEffect(() => {
    ...
    if (userEmail !== inviteEmail) {
      setState('ERROR');
      setErrorMsg(`Denne invitasjonen er sendt til ${invitation.co_landlord_email}, men du er logget inn som ${user.email}.`);
      return;
    }
  ...
  const handleAccept = async () => {
    ...
    await base44.entities.CoLandlordInvitation.update(invitation.id, {
      status: 'accepted',
    ...
    if (!user.user_role || user.user_role !== 'landlord') {
      await base44.auth.updateMe({ user_role: 'landlord', active_role: 'landlord' });
    }

    // Notify the inviting landlord
    try {
      await base44.entities.Notification.create({
        user_id: invitation.inviting_landlord_id,
        type: 'agreement',
        title: 'Medutleier akseptert',
        message: `${user.full_name || user.email} har akseptert å bli medutleier på ${property.name}`,
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex items-center justify-center p-4">
        <Card className="max-w-sm w-full">
          <CardContent className="pt-8 pb-8 text-center space-y-4">
            ...
            <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={() => navigate(createPageUrl('Dashboard'))}>
              Gå til dashbordet
            </Button>
          </CardContent>
        </Card>
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex items-center justify-center p-4">
        <Card className="max-w-sm w-full">
          <CardContent className="pt-8 pb-8 text-center space-y-4">
            ...
            <p className="text-slate-500 text-sm">Du er allerede medutleier på denne boligen.</p>
            <Button className="w-full bg-blue-600 hover:bg-blue-700" onClick={() => navigate(createPageUrl('Dashboard'))}>
              Gå til dashbordet
            </Button>
          </CardContent>
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white flex items-center justify-center p-4">
        <Card className="max-w-sm w-full">
          <CardContent className="pt-8 pb-8 text-center space-y-4">
            ...
            <Button variant="outline" className="w-full" onClick={() => navigate(createPageUrl('Dashboard'))}>
              Tilbake til appen
            </Button>
          </CardContent>
        </Card>
      ...
      <Card className="max-w-sm w-full">
        <CardContent className="pt-8 pb-8 space-y-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              {state === 'ACCEPTING' ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
              Aksepter invitasjon
            </Button>
            <Button
              variant="outline"
