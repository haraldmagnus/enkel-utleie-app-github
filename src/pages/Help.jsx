...
const FAQ_ITEMS = [
  {
    ...
    questions: [
      {
        q: 'Hvordan inviterer jeg en leietaker?',
        a: 'Gå til en eiendom → trykk "Inviter leietaker" → skriv inn leietakerens e-postadresse. Leietakeren mottar en e-post med lenke til å opprette konto og koble seg til eiendommen din.'
      },
      {
        q: 'Kan jeg ha flere eiendommer?',
        a: 'Ja, du kan ha opptil 5 eiendommer i appen. Legg til nye via "+" på Eiendommer-siden.'
      },
      {
        q: 'Hvordan fungerer leieavtalen i appen?',
        a: 'Du kan opprette en digital leieavtale under eiendommen. Begge parter signerer digitalt inne i appen. Du kan også laste opp en eksisterende PDF-avtale.'
      },
      {
        q: 'Hva skjer når en leietaker flytter ut?',
        a: 'Du kan avslutte leieforholdet fra eiendomssiden. Husk å dokumentere tilstanden med bilder via "Utflyttingsbilder" og sammenlign med innflyttingsbildene.'
      },
    ]
  },
  {
    ...
    questions: [
      {
        q: 'Hva slags inntekter og utgifter kan jeg registrere?',
        a: 'Du kan registrere husleie, depositum, vedlikehold, reparasjoner, strøm/vann, forsikring, skatter og andre poster. Gå til Økonomi-siden og trykk "Legg til post".'
      },
      {
        q: 'Kan jeg laste ned en rapport over inntektene mine?',
        a: 'Ja! Gå til "Årsrapport" fra dashbordet. Du kan filtrere per år og eiendom, og laste ned som CSV-fil for bruk i Excel eller regnskapsprogram.'
      },
      {
        q: 'Hvordan holder jeg oversikt over betalingspåminnelser?',
        a: 'Under en eiendom finner du "Betalingspåminnelser". Her setter du opp automatiske påminnelser til leietaker om forfallsdato for husleie.'
      },
    ]
  },
  {
    ...
    questions: [
      {
        q: 'Hvordan rapporterer leietaker vedlikeholdsbehov?',
        a: 'Leietaker kan sende vedlikeholdsforespørsler direkte i appen fra sitt dashboard. Du som utleier mottar varsel og kan følge opp i vedlikeholdsloggen.'
      },
      {
        q: 'Kan jeg legge til håndverkere på en vedlikeholdsoppgave?',
        a: 'Ja, du kan legge inn kontaktinformasjon for håndverkere (navn, telefon, e-post) direkte på oppgaven, samt registrere estimert og faktisk kostnad.'
      },
    ]
  },
];
...
const TAX_GUIDE_SECTIONS = [
  ...
  },
  {
    ...
    title: 'Slik bruker du appen for skatteformål',
    content: [
...
function AccordionItem({ question, answer }) {
  ...
  return (
    <div className="border-b last:border-b-0">
      <button
        className="w-full text-left flex items-center justify-between py-3 gap-2"
      ...
      {open && (
        <p className="text-sm text-slate-600 pb-3 leading-relaxed">{answer}</p>
      )}
    </div>
  );
}
    ...
    <div className="pb-20">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex bg-slate-100 rounded-xl p-1 gap-1">
          <button
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'guide' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
            onClick={() => setActiveTab('guide')}
          >
            Skatteguide
          </button>
          <button
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'faq' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500'}`}
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
              <p className="text-sm text-blue-800 leading-relaxed">
                <strong>Obs:</strong> Denne guiden er ment som generell veiledning. Skatteregler kan endre seg. Kontakt Skatteetaten eller en regnskapsfører ved tvil.
              </p>
            </div>
            ...
            <Card className="bg-white shadow-sm">
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <MessageSquare className="w-4 h-4 text-slate-500" /> Nyttige lenker
                </CardTitle>
              </CardHeader>
