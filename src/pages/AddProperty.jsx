...
export default function AddProperty() {
  ...
  const [formData, setFormData] = useState({
    name: '', address: '', description: '', monthly_rent: '',
    finn_code: '', property_type: '', size_sqm: '', bedrooms: '',
    floor: '', facilities: [], tax_type: 'secondary'
  });
  ...
  const handleFinnImport = (data) => {
    setFormData({
      ...formData,
      name: data.name || '', address: data.address || '', description: data.description || '',
      monthly_rent: data.monthly_rent?.toString() || '', finn_code: data.finn_code || '',
      property_type: data.property_type || '', size_sqm: data.size_sqm?.toString() || '',
      bedrooms: data.bedrooms?.toString() || '', floor: data.floor || '', facilities: data.facilities || []
    });
  };
  ...
  const handleSubmit = (e) => {
    ...
    createMutation.mutate({
      ...
      landlord_id: user.id, status: 'vacant'
    });
  };
        ...
        {!canAddMore ? (
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <p className="text-slate-500 text-sm">Du har allerede 5 eiendommer registrert.</p>
            </CardContent>
          </Card>
        ) : (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="F.eks. Leilighet Sentrum" required />
                </div>
                <div>
                  <Label htmlFor="address">{t('address')} *</Label>
                  <Input id="address" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} placeholder="Gate 123, 0000 By" required />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="monthly_rent">{t('monthlyRent')} (kr)</Label>
                    <Input id="monthly_rent" type="number" value={formData.monthly_rent} onChange={(e) => setFormData({ ...formData, monthly_rent: e.target.value })} placeholder="12000" />
                  </div>
                  <div>
                    <Label htmlFor="size_sqm">Størrelse (m²)</Label>
                    <Input id="size_sqm" type="number" value={formData.size_sqm} onChange={(e) => setFormData({ ...formData, size_sqm: e.target.value })} placeholder="65" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Input id="property_type" value={formData.property_type} onChange={(e) => setFormData({ ...formData, property_type: e.target.value })} placeholder="Leilighet" />
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <Textarea id="description" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="Beskriv eiendommen..." rows={3} />
                </div>
                {formData.facilities?.length > 0 && (
                  <div>
                    <Label>Fasiliteter</Label>
                    <div className="flex flex-wrap gap-2 mt-2">
                      {formData.facilities.map((f, i) => <Badge key={i} className="bg-blue-100 text-blue-700">{f}</Badge>)}
                    </div>
                  </div>
                )}
              </CardContent>
