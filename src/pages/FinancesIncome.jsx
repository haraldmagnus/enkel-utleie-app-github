...
export default function FinancesIncome() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['finances-income'] })
  });
      ...
      <div className="p-4 space-y-4">
        <Card className="bg-green-50 border-green-200">
          <CardContent className="p-3 text-center">
            ...
            <p className="text-xs text-green-600">Totale inntekter</p>
            <p className="font-bold text-green-700">{total.toLocaleString()} kr</p>
          </CardContent>
        </Card>

        <div className="space-y-2">
          {isLoading ? (
            <p className="text-slate-500 text-center py-8">Laster...</p>
          ) : entries.length === 0 ? (
            <Card><CardContent className="p-8 text-center">
              <TrendingUp className="w-12 h-12 text-slate-300 mx-auto mb-3" />
              <p className="text-slate-500">Ingen inntekter registrert ennå</p>
            </CardContent></Card>
          ) : entries.map(entry => {
              ...
              <Card key={entry.id}>
                <CardContent className="p-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center text-lg bg-green-100">
                    ...
                    <Button variant="ghost" size="icon" className="text-slate-400 hover:text-red-600" onClick={() => deleteMutation.mutate(entry.id)}>
