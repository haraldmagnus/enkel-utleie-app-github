    ...
    <div className="pb-20">
      <div className="bg-white border-b px-4 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Dokumentasjon</h1>
            ...
            {moveInPhotos.length === 0 ? (
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-slate-500">Ingen innflyttingsbilder lastet opp</p>
                </CardContent>
              </Card>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                {moveInPhotos.map((url, idx) => (
                  <a 
                    key={idx} 
                    href={url} 
                    target="_blank" 
          ...
          <TabsContent value="move_out">
            {moveOutPhotos.length === 0 ? (
              <Card>
                <CardContent className="p-8 text-center">
                  <Image className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                  <p className="text-slate-500">Ingen utflyttingsbilder lastet opp</p>
                </CardContent>
              </Card>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                {moveOutPhotos.map((url, idx) => (
                  <a 
                    key={idx} 
                    href={url} 
                    target="_blank" 
